            ...
            .map(|result| {
                result.map(|op| {
                    reencode_instruction_to_static(reencoder, op)
                        .expect("failed to reencode instruction")
                })
            })
        ...
        while let Some((instruction_idx, inst)) = instructions_iter.peek().copied() {
            'add_instructions: {
                if let Some((special_idx, plugin_import)) = special_iter.peek().copied().copied() {
                    debug_assert!(
                        instruction_idx <= special_idx as usize,
                        "next special_idx is before the instruction index, meaning it'll never get picked. did you forget to call special_iter.next()?"
                    );
